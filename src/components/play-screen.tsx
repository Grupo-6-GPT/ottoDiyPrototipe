...
import { Play, Square, RotateCcw, Layers, WifiOff, AlertCircle, ChevronRight } from 'lucide-react';
...
export function PlayScreen() {
  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ background: '#0B0B14' }}>
      ...
      {/* Header */}
      <div className="flex-shrink-0 px-5 pt-5 pb-2 text-center">
        ...
        <p style={{ color: '#4A4A6A', fontSize: 12 }}>
          {noSteps
            ...
            : `${fmt(totalDuration)} · ${steps.length} paso${steps.length !== 1 ? 's' : ''}`}
          {isDancing && ` · ${fmt(elapsed)} transcurrido`}
        </p>
      </div>

      {/* Robot */}
      <div className="flex-shrink-0 flex justify-center px-5 py-3">
        <motion.div
      ...
      {/* Step chip */}
      <div className="flex-shrink-0 px-5 flex flex-col items-center gap-2 mb-1">
        <AnimatePresence mode="wait">
          {isDancing && currentStepData && (
            ...
            >
              ...
              <span style={{ color: currentStepData.color, fontSize: 13, fontWeight: 600 }}>
                {currentStepData.name}
              </span>
              <div className="w-px h-3.5" style={{ background: '#1E1E30' }} />
              <span style={{ color: '#4A4A6A', fontSize: 11, fontWeight: 600 }}>
        ...
        <div className="flex-shrink-0 px-5 mb-3">
          <div className="flex justify-between mb-1">
            <span style={{ color: '#5A5A7A', fontSize: 10, fontWeight: 600 }}>
              {isDancing ? 'Progreso real' : 'Completado'}
            </span>
            ...
            {steps.map((step, i) => {
              ...
              return (
                ...
                >
                  <motion.span
                    style={{
                      color: isActive ? step.color : isDone ? '#2A2A40' : '#3A3A5A',
                  ...
                  <span className="flex-1" style={{
                    color: isActive ? '#E8E8F0' : isDone ? '#2A2A40' : '#7A7A98',
                    fontSize: 12, fontWeight: isActive ? 600 : 400,
                    textDecoration: isDone && !isFinished ? 'line-through' : 'none',
                  }}>{step.name}</span>

                  <div className="text-right" style={{ minWidth: 60 }}>
                    <span style={{ color: isActive ? step.color : '#3A3A5A', fontSize: 10 }}>
                      {fmt(stepDur)}
                    </span>
                    {step.repetitions > 1 && (
                      <span style={{ color: '#3A3A5A', fontSize: 9 }}> ×{step.repetitions}</span>
                    )}
                  </div>
                </motion.div>
              );
      ...
      <div className="flex-shrink-0 px-5 pb-[5.5rem] pt-2">
        ...
        {!noSteps && !isDancing && connected && (
          <p className="text-center mt-2" style={{ color: '#2E2E48', fontSize: 10 }}>
            Cada comando se envía cuando el robot termina el anterior
          </p>
        )}
      </div>
    </div>
